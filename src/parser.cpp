...
auto const identifier =
    bp::lexeme[letters_and_underscore > *(letters_and_underscore | bp::char_('0', '9'))];
...
bp::symbols<NodeType> const cmp_op = {{"==", NodeType::EQUAL}, {"!=", NodeType::NEQUAL}};

bp::symbols<NodeType> const logical_op = {{"AND", NodeType::AND}, {"OR", NodeType::OR}};

bp::symbols<NodeType> const assign_op = {{"=", NodeType::ASSIGN_OP}};
...
auto _parse(const std::string& input, bp::rule<RuleType, NodePtr> rule) -> NodePtr
{
    std::ostringstream       error_stream;
    bp::stream_error_handler error_handler("input", error_stream);
    auto const result = bp::parse(input, bp::with_error_handler(rule, error_handler), bp::ws);
...
auto parse(const std::string& input) -> NodePtr { return _parse(input, command); }
